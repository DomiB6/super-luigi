(() => {
  ...
  const CHARACTERS = [
    ...
    {id:'robi', name:'Robi', desc:'Messi von Italien', colors:['#2d6cdf','#f1c7a5','#e6e6e6'], boss:null},
  ...
  let state = 'start'; // start, select, ready, playing, modal, ending
  let selectedIndex = 0;
  let selected = null;
  let modalText = '';
  let cameraX = 0;
  ...
  function drawStart(){
    ...
    text('Klicke / tippe irgendwo',160,157,7,'#e2e8f0','center'); text('V3',303,169,5,'rgba(255,255,255,.55)','right');
  }
  ...
  addEventListener('keydown',e=>{
    ...
    if(['arrowleft','arrowright',' ','x','enter'].includes(k))e.preventDefault();
    if(state==='start'){ hardStopMusic('startTheme', true); state='select'; setMusic('menuSelect', { restart:true }); return; }
    ...
    if(state==='select'){
      if(k==='arrowleft'){selectedIndex=(selectedIndex+CHARACTERS.length-1)%CHARACTERS.length;}
      if(k==='arrowright'){selectedIndex=(selectedIndex+1)%CHARACTERS.length;}
      if(k==='arrowup'){selectedIndex=(selectedIndex-4+CHARACTERS.length)%CHARACTERS.length;}
      if(k==='arrowdown'){selectedIndex=(selectedIndex+4)%CHARACTERS.length;}
  ...
  document.querySelectorAll('#touch-controls button').forEach(btn=>{
    ...
    const down=e=>{e.preventDefault(); if(state==='ready'){beginGame();return;} if(state!=='playing')return; if(a==='left'||a==='right')keys[a]=true; if(a==='jump'){if(!keys.jump)jumpPressed=true;keys.jump=true;} if(a==='shoot'){if(!keys.shoot)shootPressed=true;keys.shoot=true;}};
    const up=e=>{e.preventDefault(); if(a==='left'||a==='right')keys[a]=false;if(a==='jump')keys.jump=false;if(a==='shoot')keys.shoot=false;};
    btn.addEventListener('pointerdown',down); btn.addEventListener('pointerup',up);btn.addEventListener('pointercancel',up);btn.addEventListener('pointerleave',up);
  });

  // STARTSCREEN-MUSIK
  // Sofort beim Laden anfordern. Browser mit erlaubtem Autoplay spielen sie direkt.
  // Wichtig: Der Fallback-Listener läuft in der CAPTURE-Phase, also noch bevor
  // ein Startscreen-Klick den Zustand auf die Charakterwahl setzt.
  function requestStartTheme() {
    if (state === 'start') setMusic('startTheme');
  }
  ...
  // Falls der Tab wieder sichtbar wird und wir noch auf dem Startscreen sind,
  // nochmals versuchen (hilft bei Browsern, die Autoplay später freigeben).
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden && state === 'start') requestStartTheme();
  });
