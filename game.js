(() => {
  ...
  // ========================================================================
  // V8 AUDIO-MANAGER
  // Hintergrundmusik läuft ab jetzt über GENAU EIN dauerhaftes Audio-Element.
  // Dieses Element wird durch den ersten "Bitte klicken"-Tap freigeschaltet
  // und danach nur noch auf eine andere Musikdatei umgeschaltet.
  // Dadurch können sich Musiktracks nicht mehr überlagern und Victory/GameOver
  // funktionieren auf Mobile/iOS deutlich zuverlässiger.
  // ========================================================================
  const MUSIC_TRACKS = new Set(['startTheme','menuSelect','gameTheme','gameOver','victory']);
  const LOOPING_MUSIC = new Set(['startTheme','menuSelect','gameTheme']);

  // Ein einziger Player für ALLE Hintergrundmusiken.
  ...
  let wantedMusicName = null;
  let musicSwitchId = 0;

  // Soundeffekte bleiben bewusst separate Player, da sie gleichzeitig mit
  // der Hintergrundmusik abgespielt werden sollen.
  const audios = {};
  for (const [name, src] of Object.entries(AUDIO)) {
    if (MUSIC_TRACKS.has(name)) continue;
  ...
  function playSfx(name) {
    ...
    try {
      ...
      if (p && typeof p.catch === 'function') {
        p.catch(err => console.warn(`[SFX] ${name} konnte nicht abgespielt werden:`, err));
      }
    } catch (err) {
      console.warn(`[SFX] ${name} Fehler:`, err);
    }
  }

  function stopAllMusic({ rewind = true } = {}) {
    ++musicSwitchId;
    wantedMusicName = null;
    currentMusicName = null;
    try {
      musicPlayer.pause();
      if (rewind) musicPlayer.currentTime = 0;
    } catch (_) {}
  }

  // Kompatibilitätsfunktion für bestehende Stellen im Spiel.
  // Da es nur noch EINEN Musikplayer gibt, wird nur gestoppt, wenn genau der
  // angegebene Track momentan aktiv/gewünscht ist.
  function hardStopMusic(name, rewind = true) {
    if (currentMusicName !== name && wantedMusicName !== name) return;
    stopAllMusic({ rewind });
  }

  async function setMusic(name, { restart = false } = {}) {
    if (!MUSIC_TRACKS.has(name) || !AUDIO[name]) {
      console.warn(`[MUSIC] Unbekannter Track: ${name}`);
      return;
    }

    // Derselbe Track soll z.B. beim Navigieren in der Charakterwahl einfach
    // weiterlaufen und NICHT bei jedem Tastendruck von vorne beginnen.
    if (!restart && currentMusicName === name && !musicPlayer.paused) {
      wantedMusicName = name;
      return;
    }

    wantedMusicName = name;
    const switchId = ++musicSwitchId;
    const src = AUDIO[name];

    try {
      // Ein alter Track wird immer zuerst vollständig beendet.
      musicPlayer.pause();

      const absoluteSrc = new URL(src, window.location.href).href;
      const sourceChanged = musicPlayer.src !== absoluteSrc;
      ...
      musicPlayer.loop = LOOPING_MUSIC.has(name);
      currentMusicName = name;

      const p = musicPlayer.play();
      if (p && typeof p.then === 'function') await p;

      // Falls während load()/play() bereits auf einen anderen Screen gewechselt
      // wurde, darf dieser veraltete Aufruf nicht wieder Musik einschalten.
      if (switchId !== musicSwitchId || wantedMusicName !== name) {
        musicPlayer.pause();
        return;
      }

      console.info(`[MUSIC] ▶ ${name} (${src})`);
    } catch (err) {
      // Nicht mehr lautlos verschlucken: Ein fehlendes/gesperrtes Audio ist
      // jetzt direkt in der Browser-Konsole erkennbar.
      console.warn(`[MUSIC] ${name} konnte nicht abgespielt werden (${src}):`, err);
    }
  }
  ...
  // Hilfreiche Diagnose für GitHub Pages / Mobile Browser.
  musicPlayer.addEventListener('error', () => {
    const code = musicPlayer.error?.code;
    console.warn(`[MUSIC] Datei konnte nicht geladen werden: ${musicPlayer.currentSrc || musicPlayer.src} (MediaError ${code ?? 'unbekannt'})`);
  });
  ...
  function beginGame() {
    ...
    state='playing';
    // Gleicher Music-Player, neue Quelle: dadurch kann die Menü-Musik niemals
    // parallel zur Game-Musik weiterlaufen.
  ...
  function finish(win) {
    state='ending'; endTimer=0;
    ending = { win, name:selected.name };
    // Kein neuer Audio-Player: derselbe, bereits beim ersten Tap freigeschaltete
    // Music-Channel wechselt direkt zu Victory bzw. Game Over.
    setMusic(win ? 'victory' : 'gameOver', { restart:true });
  }
  ...
  function clickCanvas(e){
    ...
    if(state==='start'){ state='select'; setMusic('menuSelect', { restart:true }); return; }
  ...
  // Manche Mobile-Browser pausieren Audio beim App-/Tab-Wechsel. Beim Zurückkehren
  // darf nur der zum aktuellen Screen gehörende Track fortgesetzt werden.
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden && wantedMusicName && musicPlayer.paused) {
      setMusic(wantedMusicName);
    }
  });
